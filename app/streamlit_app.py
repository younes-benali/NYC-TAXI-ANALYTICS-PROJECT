...
def main():
    ...
    with tabs[0]:
        ...
        fig_fare = px.histogram(filtered, x='fare_amount', nbins=50, title="Trip Fares",
        ...
        with col_left:
            ...
            hourly = filtered.groupby('pickup_hour').size().reset_index(name='count')
        ...
        with col_right:
            ...
            payment_counts = filtered['payment_type'].map({1:'Credit',2:'Cash',3:'No charge',4:'Dispute',5:'Unknown'}).value_counts()
        ...
        scatter_df = filtered.sample(min(5000, len(filtered)), random_state=42)
